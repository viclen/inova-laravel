...
function run()
{
    ...
    $data = file_get_contents('./storage/app/marcas_carros.json');
    ...
    foreach ($array_marcas as $dados_marca) {
        ...
        foreach ($carros as $carro_id => $carro_nome) {
            ...
            try {
                    ...
                    foreach ($dados_fipe as $dados) {
                        try {
                            ...
                            if (array_search($modelo_id, $modelos_buscados) === false) {
                                ...
                                $dados_modelo['preco'] = onlyNumbers($dados_modelo['preco']);
                                $modelo = new FipeModelo([
                                    'fipe_id' => $modelo_id,
                                    'preco' => onlyNumbers($dados_modelo['preco']),
                            ...
                            echo tab(3) . "Erro " . ($erros_modelo + 1) . "/$max_erros: $url";
                ...
                echo tab(3) . "Erro " . ($erros_carro + 1) . "/$max_erros: " . str_replace(['file_get_contents', '(', ')'], '', $th->getMessage());
