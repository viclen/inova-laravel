<template>
  <div class="container">
    <div class="row mb-1">
      <div class="col">
        <a :href="`/clientes/${cliente.id}/carros`" class="btn btn-light">
    ...
    <div class="row">
      <div class="col-12">
        <b-card header="Cadastro de Carro para Troca">
          <b-form-group>
            <label>Marca</label>
            <v-select
...
<script>
export default {
  props: ["caracteristicas", "carros", "marcas", "dados", "cliente"],
  data() {
    return {
      opcoesMarcas: [],
      erros: [],
  ...
  methods: {
    ...
    salvar() {
      ...
      let url = `/clientes/${this.cliente.id}/carros`;
      axios.post(url, dados).then(r => {
        if (r.data.status == "1") {
          let toast = this.$toasted.success("Carro salvo!", {
          ...
          setTimeout(() => {
            window.location = `/clientes/${this.cliente.id}`;
          }, 500);
        } else {
